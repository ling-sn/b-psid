...
def main(
   ...
   aligner_type: str, genome_idx: str, filter_idx: str, 
   library: str, two_pass: bool, emit_dedup: str, 
   ...
   * Automatically appends new task containing sample name to SBATCH script,
   ...
   try:
      for index, subfolder in enumerate(start_dir.iterdir()):
         if subfolder.is_dir():
            ...
            with open(output, "a") as f:
               ...
               task = (
                  f'"python3 -u run_align.py --input {input_folder} --output {output_folder} '
...
if __name__ == "__main__":
   parser = argparse.ArgumentParser(
      description = "Writes SBATCH script for STAR alignment."
   )
   parser.add_argument(
      "--input_folder", 
      help = "Name of folder containing all trimmed reads", 
      required = True
   )
   parser.add_argument(
      "--output_folder", 
      help = "Name of output folder for aligned reads (after running run_align)", 
      default = "star_aligned"
   )
   parser.add_argument(
      "--aligner_type", 
      help = "Specify aligner type", 
      choices = ["hisat2", "star"], 
      required = True
   )
   parser.add_argument(
      "--genome_idx", 
      help = "Directory to genome index", 
      required = True
   )
   parser.add_argument(
      "--filter_idx", 
      help = "Directory to contaminants index, including prefix of index "
      "(e.g., everything up to .1.bt2)"
   )
   parser.add_argument(
      "--library", 
      help = "Specify which strand is on Read 1. "
   ...
   parser.add_argument(
      "--emit_dedup", 
      help = "Name of output SBATCH file for deduplication "
      "(next step of pipeline)"
   )
   parser.add_argument(
      "--email", 
   ...
   main(
      ...
      args.aligner_type, args.genome_idx, args.filter_idx, 
      args.library, args.two_pass, args.emit_dedup,
