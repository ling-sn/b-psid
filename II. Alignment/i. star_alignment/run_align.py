...
* Runs HISAT2/STAR aligner, optionally filters against contaminant index, 
and optionally dedups with UMIs. 
...
def select_fastqs(sample_dir: Path, priority_order):
   """
   Select paired FASTQs for alignment.

   priority_order example:
   ["filtered", "fastp", "cutadapt"]
   """
   tag_map = {
      "filtered": "_filtered_",
      "fastp": "_fastp_",
...
def acquire_lock(lock_path: Path, wait_seconds = 5):
   while True:
      try:
         fd = lock_path.open("x")
         ...
         print(f">>> Lock exists ({lock_path.name}); waiting...")
...
def write_dedup_slurm(out_file, bam_files, threads, email, account):
   ...
   try:
      mem = threads * 3000
      new_tasks = [
         f"\"python parallel_dedup.py --bam {bam} -C {threads}\""
...
if __name__ == "__main__":
   parser = argparse.ArgumentParser(
      description = "Align per-sample paired FASTQs (fastp preferred, "
      "cutadapt fallback) using HISAT2 or STAR."
   )
   ...
   required.add_argument(
      "--index", 
      required = True, 
      help = "HISAT2 index prefix or STAR genomeDir."
   )

   ## Basic arguments
   data_opt = parser.add_argument_group(
      "Basic Arguments", "These options can be used to change how the script is generally run."
   )
   data_opt.add_argument(
      "-C", "--threads", 
      type = int, 
      default = 4, 
      help = "Specify the number of cpus. Default is 4"
   )
   data_opt.add_argument(
      "-L", "--library_type", 
      choices = ["RF", "FR", "unstranded"], 
      default = "unstranded", 
      help = "Specify which strand is on read 1 (RF is AS, FR is S). "
      "Default is unstranded (but NEBNext needs RF)"
   )
   data_opt.add_argument(
      "-O", "--overwrite", 
      action = "store_true", 
      help = "Specify to overwrite existing outputs"
   )
   data_opt.add_argument(
      "-S", "--sample", 
      help = "Process only a single sample directory within the input directory")
   data_opt.add_argument(
      "--fastq-priority", 
      nargs = "+", 
      choices = ["filtered", "fastp", "cutadapt"], 
      default = ["filtered", "fastp", "cutadapt"], 
      help = "Specify priority order when selecting FASTQs "
      "(default: filtered fastp cutadapt)"
   )
   data_opt.add_argument(
      "--extra-aligner-args", 
   ...
   ## Filter option arguments
   filter_opt = parser.add_argument_group(
      "Alignment-based filtering", 
      "Align against a limited reference and keep only unaligned reads "
      "for downstream processing."
   )
   filter_opt.add_argument(
      "-F", "--filter_index", 
      help = "If set, align reads to this bowtie2 index and retain "
      "unaligned reads for downstream processing."
   )
   filter_opt.add_argument(
      "-f", "--filter_only", 
      action = "store_true", 
      help = "Only perform alignment-based filtering and do not run "
      "final alignment."
   )

   ## STAR option arguments
   star_opt = parser.add_argument_group(
      "STAR options", 
      "Options specific to STAR alignment."
   )
   star_opt.add_argument(
      "-T", "--star_twopass", 
      action = "store_true", 
      help = "Enable STAR 2-pass alignment "
      "(recommended for transcript discovery)"
   )
   star_opt.add_argument(
      "-D", "--star_discovery", 
      action = "store_true", 
      help = "Enable more discovery-friendly STAR settings "
      "(at expense of more ambiguous mapped reads)"
   )

   ## Dedup option arguments
   dedup_opt = parser.add_argument_group(
      "Dedup scripting", 
      "These options can be used to help setup dedup with umi tools."
   )
   parser.add_argument(
      "--emit_dedup_slurm", 
      metavar = "FILE", 
      help = "Write a SLURM array script to deduplicate all produced BAMs"
   )
   parser.add_argument(
      "--slurm_email", 
   ...
   for sample_dir in list_sample_dirs(input_dir):
      ...
      try:
         r1, r2, stage = select_fastqs(sample_dir, args.fastq_priority)
      ...
      if args.filter_index:
         ...
         filtered_r1 = next(
            sample_dir.glob(f"{sample}*_filtered_R1.fastq.gz"), 
            None
         )
         filtered_r2 = next(
            sample_dir.glob(f"{sample}*_filtered_R2.fastq.gz"), 
            None
         )

         if filtered_r1 and filtered_r2 and not args.overwrite:
            print(
               f">>> Found existing filtered FASTQs for {sample}, "
               "skipping contaminant filter"
            )
            ...
            (filtered_prefix.with_suffix(".1")).rename(
               sample_dir / f"{sample}_{pre_filter_tag}_filtered_R1.fastq"
            )
            (filtered_prefix.with_suffix(".2")).rename(
               sample_dir / f"{sample}_{pre_filter_tag}_filtered_R2.fastq"
            )
