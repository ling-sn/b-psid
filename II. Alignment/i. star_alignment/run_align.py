...
def write_dedup_slurm(out_file, bam_files, threads, email, account):
	...
	try:
		mem = threads * 3000
		new_tasks = [f"\"python parallel_dedup.py --bam {bam} -C {threads}\""
...
if __name__ == '__main__': # allows another python script to import the functions
	...
	data_opt.add_argument("--fastq-priority", nargs="+", choices=["filtered", "fastp", "cutadapt"], default=["filtered", "fastp", "cutadapt"], help="Specify priority order when selecting FASTQs (default: filtered fastp cutadapt)")
