...
2. Create an SBATCH by running `write_slurm_alignment.py` in Bash with the following input commands:
   ...
   * **--emit_dedup:** Name of output SBATCH file for deduplication. This file is automatically generated with the name "SBATCHSubArr-DEDUP.sbatch" after STAR alignment.
...
4. After alignment concludes, deduplicate reads with the SBATCH that was automatically generated in the same directory:
   ...
   ```
   **Output:** BAM files with the "dedup" suffix (excluding file extension). These are saved in 📁 `star_aligned`.
...
* **--emit_dedup_slurm:** Name of output SBATCH file for deduplication. This file is automatically generated with the name "SBATCHSubArr-DEDUP.sbatch" after STAR alignment.
