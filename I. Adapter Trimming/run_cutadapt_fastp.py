...
def run_cutadapt_single_sample(
   sample, r1, r2, output_dir, adapter_fasta, 
   threads = 2, 
   error_rate = 0.1, 
   minimum_length = 30, 
   overlap = 8, 
...
def run_fastp_single_sample(
   sample, r1, r2, output_dir, 
   umi = True, umi_loc = "per_read", umi_len = 12, 
   threads = 2, window_trim = 4, meanQ_trim = 15, 
   min_read_length = 50, meanQ_total = 15, pcent_passQ = 30, 
   adapters_already_trimmed = True
):
...
if __name__ == "__main__":
   parser = argparse.ArgumentParser(
      description = "Runs fastP", 
      usage = '%(prog)s [options] --input in_dir --output out_dir',
      add_help = False
   )
   ...
   required.add_argument(
      "--input", 
      help = "Path to directory with paired fastqs.", 
      required = True
   )
   required.add_argument(
      "--output",
      help = "Path to new directory to place UMI_labeled fastqs.", 
      required = True
   )
   ...
   data_opt = parser.add_argument_group(
      "Basic Arguments", 
      "These options can be used to change how the script is generally run."
   )
   data_opt.add_argument(
      "-X",
      "--cutadapt_mode", 
      action = "count",
      help = "Control Cutadapt behavior (default = Cutadapt first): "
      "use -X for Cutadapt only, -XX to skip Cutadapt."
   )
   data_opt.add_argument(
      "-C",
      "--threads",
      type = int,
      default = 2,
      help = "Specify number of cpus. Default is 2."
   )
   data_opt.add_argument(
      "-S",
      "--sample", 
      help = "Process only a single sample prefix within the input directory"
   )

   ## Cutadapt options
   cut_opt = parser.add_argument_group(
      "Cutadapt Options", 
      "These options can be used to change how cutadapt runs."
   )
   cut_opt.add_argument(
      "-b", 
      "--barcode_fasta",
      default = "ARL_adapters.fa",
      help = "Specify barcode primers fasta. Default is ARL_adapters.fa"
   )
   cut_opt.add_argument(
      "-f",
      "--Tn_fasta",
      default = "Tn_adapters.fa",
      help = "Specify Tn mosaic ends fasta. Default is Tn_adapters.fa"
   )
   cut_opt.add_argument(
      "-e",
      "--cut_error",
      type = float,
      default = 0.1,
      help = "Specify max error rate when finding adapters to trim. Default = 0.1"
   )
   cut_opt.add_argument(
      "-m",
      "--min_cutadapt_read",
      type = int,
      default = 30,
      help = "Specify minimum read length to keep after adapter trimming. Default = 30"
   )
   cut_opt.add_argument(
      "-o",
      "--overlap",
      type = int,
      default = 8,
      help = "Specify minimum read-adapter overlap for trimming. Default = 8"
   )
   ...
   ## Fastp option arguments
   fastp_opt = parser.add_argument_group(
      "Fastp Options", 
      "These options can be used to change how fastp runs."
   )
   fastp_opt.add_argument(
      "-W",
      "--window_trim", 
      type = int,
      default = 4,
      help = "Specify the window size when end trimming. "
      "Default is 4"
   )
   fastp_opt.add_argument(
      "-T",
      ...
      type = int,
      default = 15,
      help = "Specify the mean Q of a window when trimming necessary to keep." 
      "Default is 15"
   )
   fastp_opt.add_argument(
      "-L",
      ...
      type = int,
      default = 30,
      help = "Specify minimum read length to keep. "
      "Default = 30"
   )
   fastp_opt.add_argument(
      "-M",
      ...
      type = int,
      default = 15,
      help = "Specify Q score to serve as keep threshold. "
      "Default is 15"
   )
   fastp_opt.add_argument(
      "-P",
      ...
      type = int,
      default = 30,
      help = "Specify percent of read nucleotides that must pass meanQ to keep. "
      "Default is 30"
   )
   fastp_opt.add_argument(
      "-U",
      ...
      type = int,
      default = None,
      help = "Length of UMI at 5' end of reads. Enables UMI extraction if set."
   )

   ## Help option
   help_opt = parser.add_argument_group("Help")
   help_opt.add_argument(
      "-h", "--help", 
      action = "help", 
   ...
   if args.sample:
      if args.sample not in pairs:
         sys.exit(
            f">>> Requested sample '{args.sample}' not found in {args.input}"
         )
